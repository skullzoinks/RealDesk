...
class VirtualXboxController extends StatefulWidget {
  const VirtualXboxController({
    Key? key,
  ...
  @override
  State<VirtualXboxController> createState() => _VirtualXboxControllerState();
}

class _VirtualXboxControllerState extends State<VirtualXboxController> {
  int _buttonsMask = 0;
  double _lx = 0;
  double _ly = 0;
  double _rx = 0;
...
class _VirtualJoystickState extends State<_VirtualJoystick> {
  ...
  void _updateDelta(Offset localPosition) {
    ...
    final y = -_delta.dy / _radius; // Y is inverted for gamepad
    widget.onChange(x, y);
    setState(() {});
  }
